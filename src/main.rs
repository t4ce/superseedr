...
fn blueprint_main() {
    ...
    loop {
        if let Some(command) = shell.poll() {
            ...
            if action == BlueprintAction::Exit {
                logl::log(
                    logl::level::INFO,
                    format_args!("superseedr: trueos blueprint shell exit"),
                );
                break;
            }
            shell_prompt();
...
impl BlueprintApp {
    ...
    fn run_command(&mut self, command: &str) -> BlueprintAction {
        if command.is_empty() {
            return BlueprintAction::Continue;
        }
        ...
        match verb {
            ...
            _ => shell_line("unknown command; try: help"),
        }

        BlueprintAction::Continue
    }
    ...
    fn remove(&mut self, target: &str) {
        ...
        let Ok(id) = target.parse::<usize>() else {
            shell_line("remove expects an id or all");
            return;
        };
        let before = self.torrents.len();
        self.torrents.retain(|torrent| torrent.id != id);
...
#[cfg(feature = "trueos-blueprint")]
struct BlueprintShellInput {
    bytes: [u8; 2048],
    len: usize,
}

#[cfg(feature = "trueos-blueprint")]
impl BlueprintShellInput {
    const fn new() -> Self {
        Self {
            bytes: [0; 2048],
            len: 0,
...
#[cfg(not(feature = "trueos-blueprint"))]
mod control_service;
#[cfg(all(feature = "dht", not(feature = "trueos-blueprint")))]
mod dht;
#[cfg(all(not(feature = "dht"), not(feature = "trueos-blueprint")))]
#[path = "dht_stub.rs"]
mod dht;
#[cfg(not(feature = "trueos-blueprint"))]
mod dht_service;
...
#[cfg(not(feature = "trueos-blueprint"))]
mod storage;
#[cfg(all(feature = "synthetic-load", not(feature = "trueos-blueprint")))]
mod synthetic_load;
#[cfg(not(feature = "trueos-blueprint"))]
mod telemetry;
#[cfg(not(feature = "trueos-blueprint"))]
...
mod host {
    ...
    use crate::config::{
        ...
        effective_shared_config_selection, get_watch_path, is_shared_config_mode, load_settings,
        load_settings_for_cli, persisted_host_id_path, persisted_shared_config_path,
        resolve_command_watch_path, set_persisted_host_id, set_persisted_shared_config,
        shared_lock_path, shared_processed_path, shared_root_path, HostIdSource,
        SharedConfigSource,
    };
    #[cfg(not(feature = "trueos-blueprint"))]
    use crate::control_service::{
        apply_offline_control_request, apply_offline_purge, control_event_details,
        list_torrent_files, online_control_success_message, resolve_purge_target_info_hash,
        resolve_target_info_hash,
    };
    #[cfg(not(feature = "trueos-blueprint"))]
    ...
    #[cfg(all(test, not(feature = "trueos-blueprint")))]
    ...
    #[cfg(all(not(windows), not(feature = "trueos-blueprint")))]
    use crossterm::event::{
    ...
    pub async fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        if let Some(Commands::ShowConfigs { all }) = cli.command.as_ref() {
            ...
            if let Err(error) = process_show_configs_command(
                settings.as_ref(),
                settings_load_error,
                *all,
                output_mode,
    ...
    fn process_launcher_setup_command(
        cli: &Cli,
        ...
        match command {
            ...
            Commands::SetHostId { host_id } => {
                Some(process_set_host_id_command(host_id, output_mode))
            }
    ...
    fn print_show_configs_text(snapshot: &ShowConfigsSnapshot, show_all: bool) {
        ...
        match snapshot.settings.client_port {
            Some(port) => {
                print_value_line("settings", "client_port", "Client port", &port.to_string())
            }
    ...
    fn process_cli_request(
        ...
        match command {
                ...
                for input in expand_add_inputs(inputs) {
                    ...
                    if *validated || download_path.is_some() {
                        ...
                        let result = if shared_mode && leader_is_running {
                            ...
                            let message = apply_offline_control_request_mut(
                                &mut offline_add_settings,
                                &request,
                            )?;
                            if output_mode == OutputMode::Text {
                                print_success(
            ...
            Commands::SetSharedConfig { path } => {
                process_set_shared_config_command(path, output_mode)
            }
            ...
            Commands::Purge { targets } => {
                ...
                for target in resolved_targets {
                    let info_hash_hex = resolve_purge_target_info_hash(settings, &target)
    ...
    fn print_catalog_recovery_report(
        candidates: &[CatalogRecoveryCandidate],
        output_mode: OutputMode,
    ) {
    ...
    fn print_torrent_details(settings: &Settings, torrent: &crate::config::TorrentSettings) {
        let info_hash_hex =
            info_hash_from_torrent_source(&torrent.torrent_or_magnet).map(hex::encode);
    ...
    fn torrent_details_value(
        settings: &Settings,
        torrent: &crate::config::TorrentSettings,
    ) -> Value {
        let info_hash_hex =
            info_hash_from_torrent_source(&torrent.torrent_or_magnet).map(hex::encode);
    ...
    fn record_offline_control_journal_entry(
        request: &ControlRequest,
    ...
    mod tests {
        ...
        fn write_recovery_torrent_file(file_name: &str) -> (tempfile::TempDir, PathBuf, String) {
            ...
            let info_hash_hex = hex::encode(
                info_hash_from_torrent_bytes(&bytes).expect("recovery torrent info hash"),
            );
        ...
        fn shared_mode_without_running_leader_mutates_shared_settings_offline() {
            ...
            let host_journal_path =
                crate::persistence::event_journal::event_journal_state_file_path()
            ...
            let shared_journal_raw =
                std::fs::read_to_string(&shared_journal_path).unwrap_or_default();
        ...
        fn validate_add_download_path_rejects_regular_file() {
            ...
            let error =
                validate_add_download_path(&file_path).expect_err("regular file should fail");
        ...
        fn show_configs_without_loaded_settings_keeps_path_report_available() {
            ...
            assert!(value["descriptions"]
                ...
                .iter()
                .any(
                    |entry| entry["section"] == "settings" && entry["key"] == "settings_load_error"
                ));
        }
        ...
        fn show_configs_shared_mode_includes_shared_absolute_paths() {
            ...
            let snapshot = build_show_configs_snapshot(Some(&settings), None)
                .expect("build shared path snapshot");
        ...
        fn shared_status_now_uses_offline_snapshot_when_no_leader_is_running() {
            ...
            std::fs::write(&status_path, "{not valid json")
                .expect("write stale invalid status file");
